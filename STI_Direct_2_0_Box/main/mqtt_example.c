...
static const char *TAG = "STI_Direct";
...
static void OTAUpdate(char *filePath, char *md5){
    ...
    if(strncmp(md5,digest_str,32) == 0){
        ...
        esp_http_client_config_t ota_client_config = {
            //.url = filePath,
            .url = "http://192.168.5.1:9001/api/firmware/STI",
...
static void node_read (int channel, float value){
    ...
    switch(channel){
        case 4:
            if(value >= 0){
                value_l = (value - value_h) * 100;
            }else{
                int value_int = value * 100;
                int a = (abs(value_h)) * 100;
    ...
    switch (channel){
        case 0: 
            if(Temp_Global > 20){
                size_data = asprintf(&sending_data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\",\"Coefficiente 1\": \"%.2f\",\"Coefficiente 2\": \"%.2f\"}",
                MAC2STR(sta_mac),0,(((Temp_Global * coeff1) - (coeff2 * 22.43)) + offset_temperature_global),Hum_Global,sending_period_global,delta_temperature_global,offset_temperature_global,coeff1,coeff2);
            } else{
                size_data = asprintf(&sending_data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\",\"Coefficiente 1\": \"%.2f\",\"Coefficiente 2\": \"%.2f\"}",
                MAC2STR(sta_mac),0,(Temp_Global + offset_temperature_global),Hum_Global,sending_period_global,delta_temperature_global,offset_temperature_global,coeff1,coeff2);
            }
            //float Temp = (((Temp_Global * coeff1) - (coeff2 * 22.43)) + offset_temperature_global);
            //size_data = asprintf(&sending_data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\",\"Coefficiente 1\": \"%.2f\",\"Coefficiente 2\": \"%.2f\"}",
            //    MAC2STR(sta_mac),0,(((Temp_Global * coeff1) - (coeff2 * 22.43)) + offset_temperature_global),Hum_Global,sending_period_global,delta_temperature_global,offset_temperature_global,coeff1,coeff2);
            if(MQTT_CONNEECTED){
        ...
        break;
        case 4:
            if(coeff1 != value_h + ((float) value_l /100)){
                esp_err_t err_1;
                nvs_handle_t my_handle_1;
                err_1 = nvs_open("storage", NVS_READWRITE, &my_handle_1);
                if (err_1 != ESP_OK) {
                ...
                coeff1 = value_h + ((float) value_l /100); 
                printf("Setting Coefficient 1 for Temperature estimation\n");
                err_1 = nvs_set_i32(my_handle_1,"coeff1_H",value_h);
                printf((err_1 != ESP_OK) ? "Failed!\n" : "Done\n");
                err_1 = nvs_set_i32(my_handle_1,"coeff1_L",value_l);
                printf((err_1 != ESP_OK) ? "Failed!\n" : "Done\n");
                printf("Committing updates in NVS ... ");
                err_1 = nvs_commit(my_handle_1);
                nvs_close(my_handle_1);
        ...
        break;
        case 5:
            if(coeff2 != value_h + ((float) value_l /100)){
                esp_err_t err_1;
                nvs_handle_t my_handle_1;
                err_1 = nvs_open("storage", NVS_READWRITE, &my_handle_1);
                if (err_1 != ESP_OK) {
                ...
                coeff2 = value_h + ((float) value_l /100); 
                printf("Setting Coefficient 2 for Temperature estimation\n");
                err_1 = nvs_set_i32(my_handle_1,"coeff2_H",value_h);
                printf((err_1 != ESP_OK) ? "Failed!\n" : "Done\n");
                err_1 = nvs_set_i32(my_handle_1,"coeff2_L",value_l);
                printf((err_1 != ESP_OK) ? "Failed!\n" : "Done\n");
                printf("Committing updates in NVS ... ");
                err_1 = nvs_commit(my_handle_1);
                nvs_close(my_handle_1);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DISCONNECTED:
        ...
        MQTT_CONNEECTED=0;
        if (mqtt_connection_attemps < 11){
            mqtt_connection_attemps = mqtt_connection_attemps + 1;
        }else{
            mqtt_connection_attemps = 0;
            for(int countdown = 2; countdown >= 0; countdown--) {
                ESP_LOGI(TAG, "%d...", countdown);
...
static void Sensing_Temp_Hum(void *arg)
{
    ...
    for (;;) {
        ...
        if (Temp_Global > 20){
            Temp = (((Temp_Global * coeff1) - (coeff2 * 22.43)) + offset_temperature_global);
        } else{
            Temp = Temp_Global + offset_temperature_global;
        }
        Hum = Hum_Global;
        size = asprintf(&data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\",\"Coefficiente 1\": \"%.2f\",\"Coefficiente 2\": \"%.2f\"}",
                            MAC2STR(sta_mac),0,Temp,Hum,sending_period_global,delta_temperature_global,offset_temperature_global,coeff1,coeff2);
...
{	
    ...
    printf("Reading Offset LSB from NVS ... ");
    ...
    printf("Reading Coefficient 1 MSB from NVS ... ");
    int32_t coeff1_H = 1; 
    int32_t coeff1_L = 0; //94 
    err = nvs_get_i32(my_handle, "coeff1_H", &coeff1_H);
    switch (err) {
        case ESP_OK:
            printf("Done\n");
            printf("Offset MSB Variable = %d\n", coeff1_H);
        ...
        case ESP_ERR_NVS_NOT_FOUND:
            printf("The value is not initialized yet!\n");
            printf("Setting the value to Default Value\n");
            err = nvs_set_i32(my_handle,"coeff1_H",coeff1_H);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
            printf("Committing updates in NVS ... ");
            err = nvs_commit(my_handle);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
    ...
    printf("Reading Coefficient 1 LSB from NVS ... ");
    err = nvs_get_i32(my_handle, "coeff1_L", &coeff1_L);
    switch (err) {
        case ESP_OK:
            printf("Done\n");
            printf("Delta Temp LSB Variable = %d\n", coeff1_L);
        ...
        case ESP_ERR_NVS_NOT_FOUND:
            printf("The value is not initialized yet!\n");
            printf("Setting the value to the Default Value\n");
            err = nvs_set_i32(my_handle,"coeff1_L",coeff1_L);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
            printf("Committing updates in NVS ... ");
            err = nvs_commit(my_handle);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
    ...
    printf("Reading Coefficient 1 MSB from NVS ... ");
    int32_t coeff2_H = 0; //1 
    int32_t coeff2_L = 0; 
    err = nvs_get_i32(my_handle, "coeff2_H", &coeff2_H);
    switch (err) {
        case ESP_OK:
            printf("Done\n");
            printf("Offset MSB Variable = %d\n", coeff2_H);
        ...
        case ESP_ERR_NVS_NOT_FOUND:
            printf("The value is not initialized yet!\n");
            printf("Setting the value to Default Value\n");
            err = nvs_set_i32(my_handle,"coeff2_H",coeff2_H);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
            printf("Committing updates in NVS ... ");
            err = nvs_commit(my_handle);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
    ...
    printf("Reading Coefficient 1 LSB from NVS ... ");
    err = nvs_get_i32(my_handle, "coeff2_L", &coeff2_L);
    switch (err) {
        case ESP_OK:
            printf("Done\n");
            printf("Delta Temp LSB Variable = %d\n", coeff2_L);
        ...
        case ESP_ERR_NVS_NOT_FOUND:
            printf("The value is not initialized yet!\n");
            printf("Setting the value to the Default Value\n");
            err = nvs_set_i32(my_handle,"coeff2_L",coeff2_L);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
            printf("Committing updates in NVS ... ");
            err = nvs_commit(my_handle);
            printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
    ...
    coeff1 = coeff1_H + ((float)coeff1_L / 100);
    coeff2 = coeff2_H + ((float)coeff2_L / 100);
    ...
    printf("COEFFICIENT 1 TEMPERATURE = %f\n",coeff1);
    printf("COEFFICIENT 2 TEMPERATURE = %f\n",coeff2);
