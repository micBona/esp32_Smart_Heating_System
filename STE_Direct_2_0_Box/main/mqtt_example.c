...
static void node_read (int channel, float value){
    ...
    switch (channel){
        case 0: 
            ...
            //size_data = asprintf(&sending_data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\", , \"Coefficiente 1\": \"%.1f\",, \"Coefficiente 2\": \"%.1f\"}",
            //    MAC2STR(sta_mac),0,(((Temp_Global * coeff1) - (coeff2 * 22.43)) + offset_temperature_global),Hum_Global,sending_period_global,delta_temperature_global,offset_temperature_global,coeff1,coeff2);
            if(MQTT_CONNEECTED){
        ...
        break;
        case 4:
            if(coeff1 != value_h + ((float) value_l /10)){
                esp_err_t err_1;
                nvs_handle_t my_handle_1;
                err_1 = nvs_open("storage", NVS_READWRITE, &my_handle_1);
                if (err_1 != ESP_OK) {
                ...
                coeff1 = value_h + ((float) value_l /10); 
        ...
        break;
        case 5:
            if(coeff2 != value_h + ((float) value_l /10)){
                esp_err_t err_1;
                nvs_handle_t my_handle_1;
                err_1 = nvs_open("storage", NVS_READWRITE, &my_handle_1);
                if (err_1 != ESP_OK) {
                ...
                coeff2 = value_h + ((float) value_l /10); 
...
static void Sensing_Temp_Hum(void *arg)
{
    ...
    for (;;) {
        ...
        Hum = Hum_Global;
        size = asprintf(&data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Period\": %d, \"Delta_T\": \"%.1f\", \"Offset_T\": \"%.1f\",\"Coefficiente 1\": \"%.1f\",\"Coefficiente 2\": \"%.1f\"}",
...
{	
    ...
    coeff1 = coeff1_H + ((float)coeff1_L / 10);
    coeff2 = coeff2_H + ((float)coeff2_L / 10);
