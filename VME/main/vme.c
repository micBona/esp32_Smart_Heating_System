...
static const adc_channel_t channel = ADC_CHANNEL_3;     //GPIO34 if ADC1, GPIO14 if ADC2.  adc1_channel_t o adc2_channel_t
...
static const int RESCAN_INTERVAL = 15;
static int averageONValues = 16;
static int averageOFFValues = 16;
static int ValveState = 0;

static uint32_t HighSensorH = 0;
static uint32_t HighSensorL = 0;
static uint32_t LowSensorH = 0;
static uint32_t LowSensorL = 0;

static const char *TAG = "VME";
...
static unsigned int OpeningValve(bool setup){
    ...
    err = nvs_get_i32(my_handle, "valveState", &valveStateEEPROM);
    switch (err) {
            case ESP_OK:
                printf("Done\n");
    ...
    if(valveStateEEPROM == 0){
        ...
        gpio_set_level(GPIO_LEFT, 0);
        printf("Setting the value to 100\n");
        err = nvs_set_i32(my_handle,"valveState",100);
...
static unsigned int ClosingValve(bool setup){
    unsigned int result = 0;
    int32_t valveStateEEPROM;
    esp_err_t err;
    nvs_handle_t my_handle;
    ...
    if(valveStateEEPROM == 100){
        ...
        err = nvs_set_i32(my_handle,"valveState",0);
        printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
        printf("Committing updates in NVS ... ");
        err = nvs_commit(my_handle);
        printf((err != ESP_OK) ? "Failed!\n" : "Done\n");        
            ...
            if (count == 5){
                ...
                else
                {
                    ...
                    if(temps[0] > temps[1]){
                        ...
                        HighSensorH = addr0;
                        HighSensorL = addr1;
                        printf("  HIGH Sensor %08x%08x \n", addr0, addr1);
                        addr0 = addrs[1] >> 32;
                        addr1 = addrs[1];
                        LowSensorH = addr0;
                        LowSensorL = addr1;
                        printf("  LOW Sensor %08x%08x \n", addr0, addr1);
                    }
                    else{
                        uint32_t addr0 = addrs[1] >> 32;
                        uint32_t addr1 = addrs[1];
                        HighSensorH = addr0;
                        HighSensorL = addr1;
                        printf("  HIGH Sensor %08x%08x \n", addr0, addr1);
                        addr0 = addrs[0] >> 32;
                ...
                tmp_OFF = 0;
                    for(int j = 0; j < 3; j++){
                        tmp_ON = tmp_ON + (OpeningValve(true)/1000);
                        vTaskDelay(pdMS_TO_TICKS(1000));
                        tmp_OFF = tmp_OFF + (ClosingValve(true)/1000);
                        vTaskDelay(pdMS_TO_TICKS(1000));                        
                    }
                    tmp_ON = tmp_ON / 3;
                    tmp_OFF = tmp_OFF / 3;
                    averageONValues = tmp_ON;
                    averageOFFValues = tmp_OFF;
...
void settingMotors(){
    ...
    unsigned int tmp_OFF = 0;
    for(int j = 0; j < 3; j++){
        tmp_OFF = tmp_OFF + (OpeningValve(true)/1000000);
        vTaskDelay(pdMS_TO_TICKS(1000));
        tmp_ON = tmp_ON + (ClosingValve(true)/1000000);
        vTaskDelay(pdMS_TO_TICKS(1000));       
    }
    tmp_ON = tmp_ON / 3;
    tmp_OFF = tmp_OFF / 3;
        ...
        if (data_type.upgrade) { // This mesh package contains upgrade data.
            ...
            if (!strcmp(data, "restart")) {
                ...
                if(sensor_count > 0){
                    ...
                    uint32_t addr0 = addrs[0] >> 32;
                    uint32_t addr1 = addrs[0];
                    if(addr0 == HighSensorH && addr1 == HighSensorL){
                        HTemp = temps[0];
                        LTemp = temps[1];
                    }else{
                        HTemp = temps[1];
                ...
                switch(msg_code->valueint){
                    case 0:
                        size_data = asprintf(&sending_data,"{\"Mac Address\": \"%02x%02x%02x%02x%02x%02x\",\"CH\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Valve\": \"%.1f\", \"Sup_Temp\": \"%.1f\", \"Ret_Temp\": \"%.1f\" }",
                            MAC2STR(sta_mac),0,Temp,Hum,ValveState,HTemp,LTemp);
                    ...
                    case 1:
                        ...
                        if(value->valueint == 100){
                            ...
                            OpeningValve(false);
                            xEventGroupSetBits(event_group, CONNECTED_BIT_0);
                        }else if(value->valueint == 0){
                            printf("CLOSING VALVE\n");
                            ClosingValve(false);
                            xEventGroupClearBits(event_group, CONNECTED_BIT_0);                        
                        }
...
static void node_write_task(void *arg)
{
    ...
    for (;;) {
        ...
        xEventGroupWaitBits(event_group, CONNECTED_BIT_0, false, true, portMAX_DELAY);
        ...
        else
        {
            ...
            for (int j = 0; j < RESCAN_INTERVAL; j++){
                float tmp_temp_H = 0;
                float tmp_temp_L = 0;
                for(int i = 0; i < 2; i++){
                    ds18x20_measure_and_read_multi(SENSOR_GPIO, addrs, sensor_count, temps);
                    //printf("Sensed Temperature.%f\n",temps[0]);
                    uint32_t addr0 = addrs[0] >> 32;
                    uint32_t addr1 = addrs[0];
                    // uint32_t addr2 = addrs[1] >> 32;
                    // uint32_t addr3 = addrs[1];
                    if(addr0 == HighSensorH && addr1 == HighSensorL){
                        tmp_temp_H = tmp_temp_H + temps[0]; 
                        tmp_temp_L = tmp_temp_L + temps[1];
                    }else{
                        tmp_temp_H = tmp_temp_H + temps[1]; 
                        tmp_temp_L = tmp_temp_L + temps[0];
                    }
                    vTaskDelay(30000 / portTICK_RATE_MS);
                }
                tmp_temp_H = tmp_temp_H/2;
                tmp_temp_L = tmp_temp_L/2;
                cJSON *item = cJSON_CreateNumber(tmp_temp_H);
                cJSON *item2 = cJSON_CreateNumber(tmp_temp_L);
                cJSON_AddItemToArray(obj, item);
                cJSON_AddItemToArray(obj2, item2);
            }      
        }
        
        char *str = cJSON_PrintUnformatted(obj);
        char *str_2 = cJSON_PrintUnformatted(obj2);

        esp_mesh_get_parent_bssid(&parent_mac);
        //ds18x20_measure_and_read_multi(SENSOR_GPIO, addrs, sensor_count, temps);

        size = asprintf(&data, "{\"Mac Address\": \"%02x%02x%02x%02x%02x%02x\", \"CH\": %d, \"VALUE\": \"{Sensor_Values_HIGH:%s,Sensor_Values_LOW:%s}\"}",
                         MAC2STR(sta_mac),4, str, str_2);
...
static void Sensing_Temp_Hum(void *arg)
{
    ...
    for (;;) {
        ...
        Hum = roundf(Hum * 10) / 10;
        Temp = roundf(Temp * 10) / 10;
        if ((fabs(Temp - tmpTemp) >= 0.1) || sendingTimes == 6){
            printf("BEFORE SENDING FREE HEAP SIZE SENSING TASK: %lu\n", (unsigned long)esp_get_free_heap_size());
            size = asprintf(&data,"{\"Mac Address\": \"%02x%02x%02x%02x%02x%02x\",\"CH\":%d, \"Temp\": \"%.1f\", \"Hum\": \"%.1f\", \"Valve\": \"%.1f\", \"Sup_Temp\": \"%.1f\", \"Ret_Temp\": \"%.1f\" }",
                            MAC2STR(sta_mac),0,Temp,Hum,ValveState,HTemp,LTemp);
            MDF_LOGD("Node send, size: %d, data: %s", size, data);
            ret = mwifi_write(NULL, &data_type, data, size, true);
            ...
            tmpTemp = Temp; 
            sendingTimes = 0;
            printf("AFTER SENDING FREE HEAP SIZE SENSING TASK: %lu\n", (unsigned long)esp_get_free_heap_size());
        }
        else{
            free(p);
            MDF_FREE(data);
...
{	
    ...
    gpio_pad_select_gpio(GPIO_SENSE_IN);
    gpio_set_direction(GPIO_SENSE_IN, GPIO_MODE_INPUT);
    
    gpio_pad_select_gpio(GPIO_SENSE_OUT);
    gpio_set_direction(GPIO_SENSE_OUT, GPIO_MODE_OUTPUT);
    gpio_set_level(GPIO_SENSE_OUT, 0);
    ...
    // Read
        ...
        switch (err) {
            ...
            case ESP_ERR_NVS_NOT_FOUND:
                printf("The value is not initialized yet!\n");
                printf("Setting the value to 0\n");
                err = nvs_set_i32(my_handle,"valveState",valveStateEEPROM);
                printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
                printf("Committing updates in NVS ... ");
                err = nvs_commit(my_handle);
                printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
