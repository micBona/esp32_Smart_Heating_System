...
static mesh_mqtt_data_t *mesh_mqtt_parse_data(const char *topic, size_t topic_size, const char *payload, size_t payload_size)
{
    ...
    if (addr == NULL) {
        MDF_LOGE("ADDRESS IS NULL --> GOTO EXIT");
    ...
    */
    ...
    if (code == NULL) {
        MDF_LOGE("CHANNEL IS NULL --> GOTO EXIT");
        //MDF_FREE(request->addrs_list);
        MDF_FREE(request);
        goto _exit;
    }
    ...
    //cJSON *value = cJSON_GetObjectItem(obj, "value"); ONLY FOR STE AND STI
    ...
    if(code->valueint == -3){
        ...
        if (md5 == NULL) {
            MDF_LOGE("MD5 IS NULL --> GOTO EXIT");
            //MDF_FREE(request->addrs_list);
            MDF_FREE(request);
            goto _exit;
        }

        request->md5_size = strlen(md5->valuestring);
        request->md5 = MDF_MALLOC(request->md5_size);
        assert(request->md5 != NULL);
        memcpy(request->md5, md5->valuestring, request->md5_size);

        cJSON *firmware = cJSON_GetObjectItem(obj, "firmware");
        if (firmware == NULL) {
            //MDF_FREE(request->addrs_list);
            MDF_LOGE("FIRMWARE IS NULL --> GOTO EXIT");
            MDF_FREE(request->md5)
            MDF_FREE(request);
            goto _exit;
        }

        request->firmware_size = strlen(firmware->valuestring);
        request->firmware_url = MDF_MALLOC(request->firmware_size); 
        assert(request->firmware_url != NULL);
        memcpy(request->firmware_url, firmware->valuestring, request->firmware_size);
        ...
        */
    ...
    else if(code->valueint == 1 || code->valueint == 2 || code->valueint == 3){
        ...
        if (value == NULL) {
            ...
            request->value = atof(value->valuestring);    
        }
    }
    else{
        request->value = 0;
    ...
    //request->value = 0; //ONLY FOR STE AND STI
    request->addrs_num = 1;
    request->addrs_list = MDF_MALLOC(MWIFI_ADDR_LEN * request->addrs_num);           
...
mdf_err_t mesh_mqtt_write(uint8_t *addr, const char *data, size_t size, mesh_mqtt_publish_data_type_t type)
{
    ...
    cJSON *messageValue = cJSON_GetObjectItem(obj1, "Temp");
    cJSON *messageValue_1 = cJSON_GetObjectItem(obj1, "Hum");
    ...
    int len_1 = strlen(messageValue->valuestring);
    char *buffer_1 = MDF_MALLOC(len_1 + 1);
    MDF_ERROR_GOTO(buffer_1 == NULL, _no_mem, "Allocate mem failed");
    memcpy(buffer_1, messageValue->valuestring, len_1);
    buffer_1[len_1] = '\0';
    cJSON *value_s1 = cJSON_AddStringToObject(obj2, "Temp", buffer_1);
    MDF_FREE(buffer_1);
    MDF_ERROR_GOTO(value_s1 == NULL, _no_mem, "Add string to JSON failed");

    //char *payload = cJSON_PrintUnformatted(obj2);
    */
    char *payload = cJSON_PrintUnformatted(obj1);
