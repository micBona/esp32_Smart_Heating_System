    ...
    switch (channel){
        case 0:
        	if(value_h == 100 && boiler_status_global == 0){
                ...
                size_data = asprintf(&sending_data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"status\": %d}",MAC2STR(sta_mac),0,boiler_status_global);
                if(MQTT_CONNEECTED){
...
static void node_write_task(void *arg){
    ...
    for (;;) {
        ...
        size = asprintf(&data,"{\"mac\": \"%02x:%02x:%02x:%02x:%02x:%02x\",\"channel\":%d, \"status\": %d}",MAC2STR(sta_mac),0,boiler_status_global);
